...
def create(
    ...
    clip_minibatch_size: Optional[int] = 4,
...
def create(**kwargs) -> Optional['DocumentArray']:
    ...
    :param clip_minibatch_size: [DiscoArt] Evaluate image cuts using CLIP in small minibatches, this helps to save VRAM and avoid OOM when using big CLIP models such as `ViT-L-14-336::openai`. For example, `clip_minibatch_size=1` forces all CLIPs to evaluate every cut one by one. This solves most OOM issue. The only downside of small minibatch_size is being marginally slower than large minibatch_size. There is no influence on the quality of the generated images, as minibatch_size does not change the loss function.
    ...
    :param name_docarray: When specified, it overrides the default naming schema of the resulted DocumentArray. Useful when you have to know the result DocumentArray name in advance.
